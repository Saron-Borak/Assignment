...
@section('toolbar')
    <a href="{{ route('admin.reports.class-section.export', [$section, ...request()->query()]) }}"
...
<form method="GET" class="row g-2 align-items-end mb-3 no-print">
    ...
    @if ($from || $to)
        <div class="col-auto">
            <a href="{{ route('admin.reports.class-section', $section) }}" class="btn btn-sm btn-outline-secondary">Clear</a>
        </div>
    @endif
...
<x-page-card title="Attendance register" :subtitle="'Minimum required: '.config('attendance.min_percentage').'%'">
    @if ($stats->isEmpty())
        <x-empty-state icon="bi-people" title="Nobody enrolled" message="Add students to this section first." />
    @else
