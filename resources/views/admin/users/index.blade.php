                ...
                <tr>
                    ...
                    <td>
                        <span class="badge text-bg-light border">
                            <i class="bi {{ $user->role->icon() }} me-1"></i>{{ $user->role->label() }}
                        </span>
                    </td>
                    <td>
                        @if ($user->is_active)
            ...
            <form method="POST" action="{{ route('admin.users.password', $user) }}" class="modal-content">
                ...
                <div class="modal-body">
                    <p class="small text-secondary">
                        ...
                        The system does not send email, so pass the new password on directly.
                    </p>
                    <div class="mb-3">
                    ...
                    <div>
                        <label class="form-label" for="pw-confirm-{{ $user->id }}">Confirm password</label>
                        <input type="password" id="pw-confirm-{{ $user->id }}" name="password_confirmation" class="form-control" required minlength="8" autocomplete="new-password">
                    </div>
                </div>
